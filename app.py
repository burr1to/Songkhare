...
def get_token():
    ...
    if result.status_code != 200 or "access_token" not in json_result:
        raise Exception(
            f"Failed to retrieve access token: {json_result.get('error', 'No error info')}"
        )
...
def get_track_preview_url(token, track_name):
    ...
    if (
        ...
        or "tracks" not in json_result
        or "items" not in json_result["tracks"]
    ):
        print(
            f"Failed to search for track: {json_result.get('error', 'No error info')}"
        )
        return None
...
def recommend_songs_by_emotion(emotion):
    ...
    for idx, row in df.iterrows():
        ...
        if difference <= threshold and song_name not in seen_songs:
            closest_songs.append(
                (idx, song_name, valence, difference, artists, album_name)
            )
